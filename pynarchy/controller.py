...
# @Last Modified time: 2022-05-31 23:30:43
...
class Controller(QDockWidget):
    ...
    def add_raster_view(self, group, name):
        group_times = np.hstack([group[k].index.values for k in group.keys()])
        group_clusters = np.hstack([
            np.ones(len(group[k]), dtype='int')*k for k in group.keys()
            ])
